...
namespace Laba1TuringMachine
{
    class TuringMachine
    {
        ...
        public static void TuringMachineAlgorithm(char[] alphabet, char[] lenta, int lentaIndexNow, char[,,] statusTable)
        {
            int statusNow = 1;
            ...
            while (statusNow != 0)
            {
                ...
                {
                    if (lenta[lentaIndexNow].Equals(alphabet[i]))
                    {
                        alphabetLetterNow = i;
                    }
                }
